...
현재 emacs 최신 버전(emacs-29) 공식 릴리즈에서는 tree-siter를 기본적으로 포함하고 있지 않다. 그래서 =--with-tree-sitter= 옵션을 추가하여 소스 빌드해야 이 기능을 사용할 수 있다o(더불어 소스 빌드 하는 김에 실행 속도를 높이기 위해 =--with-native-compilation== 옵션도 추가하는 것이 좋음).
