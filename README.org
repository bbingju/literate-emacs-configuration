...
#+begin_src emacs-lisp
  ...
  ;; 이것이 nil이면 진행 표시줄이 같은 줄을 덮어쓸 수 있음
  ...
  (add-hook 'shell-mode-hook
            (lambda ()
              ...
              ;; carriage return 처리 활성화
              (add-hook 'comint-output-filter-functions
                        'comint-carriage-motion nil t)))
#+end_src

참고: shell-mode는 완전한 터미널 에뮬레이터가 아니므로 복잡한 진행 표시줄(APT, npm 등)은 완벽하게 표시되지 않을 수 있습니다. 더 나은 터미널 경험을 원하시면 =vterm= 사용을 권장합니다 (~M-x vterm~ 또는 ~C-c t~).
