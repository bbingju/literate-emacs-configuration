...
#+begin_src emacs-lisp
  ...
  ;; 키바인딩 설정
  ;; C-c c prefix for Claude commands (c = claude)
  (global-set-key (kbd "C-c c c") 'claude-vterm)      ; C-c c c: start Claude vterm
  (global-set-key (kbd "C-c c r") 'claude-send-region) ; C-c c r: send region
  (global-set-key (kbd "C-c c b") 'claude-send-buffer) ; C-c c b: send buffer
  (global-set-key (kbd "C-c c a") 'claude-ask)         ; C-c c a: ask question
  (global-set-key (kbd "C-c c p") 'claude-project)     ; C-c c p: project session
#+end_src
